...
public:
	...
	int addcolor(const char *name);
	// adds area symbol with name and number encoded as 4100 == 410.0 in ocad
	int addareasymbol(const char *name, int number, int color);
	int exportArea(const vector<point>&area, int symbol);
...
int doExport()
{
	OcadWriter *writer = OcadWriter::Factory(10000,10000,10000);
